...
/**
 * Notification bell with an unread-count badge and a dropdown of the
 * session's derived events. Client-side and session-only by design — the
        ...
        <div className="absolute right-0 z-50 mt-2 w-80 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lift">
          <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
            <div>
              ...
              <p className="text-[11px] text-slate-400">Session-only — resets on refresh</p>
            </div>
            <div className="flex gap-1">
              ...
              <button
                ...
                className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-slate-50 hover:text-red-500 disabled:cursor-not-allowed disabled:opacity-40"
                title="Clear all"
              >
            ...
            {events.length === 0 ? (
              <div className="px-4 py-10 text-center">
                ...
                <p className="mt-1 text-xs text-slate-400">
                  Queue updates (called, moved up, complete) will show up here.
                </p>
              </div>
            ) : (
              <ul className="divide-y divide-slate-50">
                {events.map((event) => (
                  <li key={event.id} className="flex gap-3 px-4 py-3 transition-colors hover:bg-slate-50">
                    <span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${KIND_DOT[event.kind] ?? 'bg-slate-400'}`} />
                    <div className="min-w-0">
                      <div className="flex items-baseline justify-between gap-2">
                        <p className="truncate text-sm font-semibold text-slate-800">{event.title}</p>
                        <span className="shrink-0 text-[10px] text-slate-400">{timeAgo(event.createdAt)}</span>
                      </div>
                      <p className="mt-0.5 text-xs leading-relaxed text-slate-500">{event.message}</p>
                      {!event.read && (
