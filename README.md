...
> Docker is one way to run CareQ. It is **also deployed live on Azure** (Day 15) — see the banner at the top. Docker remains the fastest way to develop, test, and demo locally.
...
| **Day 16** | **Web Push Notifications (VAPID) + per-user preferences** — browser push delivery for queue events (free, no third-party service): `push_subscriptions` + `notification_preferences` tables, `GET/PUT /api/notifications/preferences`, `POST/DELETE /api/notifications/push/subscriptions`, async fail-open `WebPushDeliveryService` (dead-endpoint self-cleanup on 404/410), `public/sw.js` service worker, bell push toggle with permission flow, VAPID keygen script + Azure/Vercel wiring | ✅ Complete |
